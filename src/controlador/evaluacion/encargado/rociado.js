...
rutas.post("/listar-municipios", async (req, res) => {
    try {
        const resultado = await rociado.listarMunicipios(req.body.municipio_id_tec, req.body.municipio_tec)
...
rutas.post("/buscar", async (req, res) => {
    try {
        const { fecha1, fecha2, municipio_id_tec, entidad, gestion } = req.body
        const datos = { fecha1, fecha2, municipio_id_tec, entidad, gestion }
