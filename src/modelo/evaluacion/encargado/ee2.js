    ...
    buscar = async (datos) => {
        ...
        const sql =
            `SELECT  
                ...
                where
                ${datos.entidad === 1000 ? ` e.municipio = ${pool.escape(datos.municipio_id_tec)}` : `e.comunidad = ${pool.escape(datos.entidad)}`}
                
                and (m.num BETWEEN ${pool.escape(datos.fecha1)} and ${pool.escape(datos.fecha2)})  and id_gestion = ${pool.escape(datos.gestion)}
