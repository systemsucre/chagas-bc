...
module.exports = class EE1 {
    ...
    filtarEvaluacionesViviendas = async (gestion, user, comunidad) => {
        const sql =
            `
            SELECT 
            id,  SUBSTR(mes, 1 , 3 ) as mes
            from ee1 
            where id_gestion = ${pool.escape(gestion)} and usuario = ${pool.escape(user)}`
        const [rows] = await pool.query(sql)
        ...
        const sqlViviendas =
            ...
            id, cv, habitantes 
            from casa
            where comunidad = ${pool.escape(comunidad)} order by CAST(cv as INT) asc`
        const [rowsViviendas] = await pool.query(sqlViviendas)

        // console.log(rowsViviendas, comunidad)
